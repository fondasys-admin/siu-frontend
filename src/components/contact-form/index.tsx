...
const defaultLabels: Required<ContactFormLabels> = {
	name: "Name",
...
export function ContactForm({
	...
	return (
		<div className="flex flex-col gap-4 items-center text-center mb-10">
			...
			<p className="text-base text-[#3c4043] leading-6 max-w-[700px]">
				{description}
			</p>
		<Form {...form}>
			<form
				onSubmit={form.handleSubmit(onSubmit)}
				className=" contents w-full md:space-y-8"
			>
				<ReCAPTCHA
					ref={recaptchaRef}
					size="invisible"
				...
				<div className="grid grid-cols-1 lg:grid-cols-2 gap-4 w-full">
									...
									<PopoverTrigger asChild>
										<FormControl>
											<Button
												...
												role="combobox"
												disabled={isLoading}
												className="peer w-full h-10 px-3 text-base focus-visible:ring-0  disabled:pointer-events-none text-black justify-between border-0 hover:bg-transparent border-b-2 rounded-none data-[state=open]:border-primary "
					...
					<FormField
						...
						render={({ field }) => (
							<FormItem className="col-span-full">
								<FormControl>
									<Textarea className="peer rounded-none border-0 border-b-2 shadow-none ring-0 focus-visible:border-primary focus-visible:ring-0 placeholder:text-transparent text-black px-3" placeholder={l.message} {...field} />
								</FormControl>
					...
					<FormField
						...
						render={({ field }) => (
							<FormItem className="flex flex-col items-center col-span-full px-4 md:px-auto">
								...
								<FormMessage className="text-sm  relative !left-auto"></FormMessage>
				...
				<Button
					type="submit"
					className="w-full md:w-auto mt-8 md:m-0 h-12 px-8 text-base"
					disabled={isLoading}
				>
					{isLoading ? (
						<>
